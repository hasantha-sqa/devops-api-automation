Feature: Delete an Existing User in the Leaderboard
  ...
  Scenario: User Wants to Verify Deleting an Existing User in the Leaderboard
    Given user enters deleting leaderboard user's username as "Testx"
    When user hits delete leaderboard user endpoint
    Then user should get delete leaderboard user response with status code 204
